...
async function openRecordings(){recordingsDialog.hidden=false;const list=$('#recordings-list'),empty=$('#recordings-empty');list.innerHTML='';empty.textContent='正在加载历史录像…';try{const data=await (await fetch(controlApi+'/api/recordings',{cache:'no-store'})).json();const items=data.items||[];empty.textContent=items.length?'':'暂无已保存录像。停止录制后，视频会显示在这里。';items.forEach(item=>{const li=document.createElement('li');const when=new Date(item.modified_at*1000).toLocaleString('zh-CN');li.innerHTML=`<span>${item.filename}<br><small>${item.size_mb} MB · ${when}</small></span><button class="download">导出视频</button><button class="delete">删除</button>`;li.querySelector('.download').onclick=()=>{const a=document.createElement('a');a.href=controlApi+'/api/recordings/'+encodeURIComponent(item.filename)+'/download';a.download=item.filename;a.click()};li.querySelector('.delete').onclick=async()=>{if(!confirm('确认删除录像 '+item.filename+'？'))return;const r=await fetch(controlApi+'/api/recordings/'+encodeURIComponent(item.filename),{method:'DELETE'});if(r.ok)openRecordings();else alert('删除失败')};list.appendChild(li)})}catch(e){empty.textContent='录像列表加载失败：'+e.message}}
$('#recordings-close').onclick=()=>recordingsDialog.hidden=true;recordingsDialog.onclick=e=>{if(e.target===recordingsDialog)recordingsDialog.hidden=true};$('#recordings-button')?.addEventListener('click',openRecordings);document.querySelector('#tab-history')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openRecordings()},true);
